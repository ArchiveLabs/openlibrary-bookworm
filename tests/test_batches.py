...
def test_create_batch(client):
    r = client.post("/v1/batches", json={"name": "test-batch", "submitter": "bot"}, headers=auth_headers())
...
def test_create_batch_requires_auth(client):
    # missing Authorization header → 401 from HTTPBearer; wrong token → 403
    r = client.post("/v1/batches", json={"name": "test-batch"})
...
def test_get_batch_empty(client):
    r = client.post("/v1/batches", json={"name": "empty"}, headers=auth_headers())
    batch_id = r.json()["id"]
...
def test_add_items_and_get_batch_counts(client):
    batch_id = client.post("/v1/batches", json={"name": "b"}, headers=auth_headers()).json()["id"]

    items = [
        {"source_id": "bwb:111", "data": {"title": "Book One"}, "status": "pending"},
        {"source_id": "bwb:222", "data": {"title": "Book Two"}, "status": "pending"},
    ]
    r = client.post(f"/v1/batches/{batch_id}/items", json=items, headers=auth_headers())
    assert r.status_code == 201
...
def test_add_items_skips_duplicates(client):
    batch_id = client.post("/v1/batches", json={"name": "dup"}, headers=auth_headers()).json()["id"]

    item = [{"source_id": "bwb:123", "data": {"title": "Dupe"}}]
    client.post(f"/v1/batches/{batch_id}/items", json=item, headers=auth_headers())
    r = client.post(f"/v1/batches/{batch_id}/items", json=item, headers=auth_headers())
    assert r.json() == {"added": 0, "skipped": 1}


def test_add_items_invalid_status(client):
    batch_id = client.post("/v1/batches", json={"name": "x"}, headers=auth_headers()).json()["id"]
    r = client.post(
...
def test_add_items_batch_not_found(client):
    r = client.post("/v1/batches/9999/items", json=[], headers=auth_headers())
    assert r.status_code == 404
