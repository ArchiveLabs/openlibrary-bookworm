ITEM = {"source": "bwb", "value": "9780451524935", "data": {"title": "1984", "source_records": ["bwb:9780451524935"]}}
ITEM2 = {"source": "bwb", "value": "9780060935467", "data": {"title": "To Kill a Mockingbird", "source_records": ["bwb:9780060935467"]}}
...
def test_add_items_validation_error(client):
    batch_id = _batch(client)
    bad = {"source": "bwb", "value": "999", "data": {}}  # missing title and source_records
...
def test_add_items_unknown_source_rejected(client):
    batch_id = _batch(client)
    bad = {"source": "unknown_source", "value": "123", "data": {"title": "x", "source_records": ["x:123"]}}
    r = client.post(f"/v1/batches/{batch_id}/items", json=[bad])
    assert r.status_code == 422  # Pydantic rejects unknown SourceType


def test_add_items_invalid_status(client):
    batch_id = _batch(client)
    item = {**ITEM, "status": "bogus"}
    r = client.post(f"/v1/batches/{batch_id}/items", json=[item])
